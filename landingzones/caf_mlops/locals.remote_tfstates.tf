...
locals {
  ...
  combined = {
    ...
    virtual_wans                     = merge(local.remote.virtual_wans, map(var.landingzone.key, module.mlops.virtual_wans))
    private_dns                      = merge(local.remote.private_dns, map(var.landingzone.key, module.mlops.private_dns))
    application_gateways             = merge(local.remote.application_gateways, map(var.landingzone.key, module.mlops.application_gateways))
    application_gateway_applications = merge(local.remote.application_gateway_applications, map(var.landingzone.key, module.mlops.application_gateway_applications))
  }
