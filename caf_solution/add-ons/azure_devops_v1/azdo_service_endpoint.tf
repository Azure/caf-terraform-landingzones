...
data "azurerm_key_vault_secret" "client_secret" {
  ...
  name         = format("%s-client-secret", local.remote.aad_apps[each.value.lz_key][each.value.aad_app_key].keyvaults[each.value.keyvault_key].secret_name_client_secret)
  key_vault_id = local.remote.aad_apps[each.value.lz_key][each.value.aad_app_key].keyvaults[each.value.keyvault_key].id
}
...
resource "azuredevops_serviceendpoint_azurerm" "azure" {
  ...
  credentials {
    serviceprincipalid  = local.remote.aad_apps[each.value.lz_key][each.value.aad_app_key].azuread_application.application_id
    serviceprincipalkey = data.azurerm_key_vault_secret.client_secret[each.key].value
  }
  azurerm_spn_tenantid      = local.remote.aad_apps[each.value.lz_key][each.value.aad_app_key].tenant_id
