resource "vault_azure_secret_backend_role" "existing_object_id" {
  ...
  application_object_id = var.objects[var.settings.azuread_application.lz_key][var.settings.azuread_application.output_key][var.settings.azuread_application.attribute_key]
