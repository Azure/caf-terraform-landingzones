    ...
    for mg_id, mg_value in try(var.archetype_config_overrides, {}) : mg_id => {
      ...
      parameters = {
        for param_key, param_value in try(mg_value.parameters, {}) : param_key => {
          for key, value in param_value : key => try(value.value, value.values, local.caf[value.output_key][value.lz_key][value.resource_type][value.resource_key][value.attribute_key])
        }
