...
{% endif %}
  -tfstate {{ config.tfstates.platform.express_route_circuit_peerings[circuit].tfstate }} \
  -env {{ config.caf_terraform.launchpad.caf_environment }} \
  -level {{ level }} \
  -p ${TF_DATA_DIR}/{{ config.tfstates.platform.express_route_circuit_peerings[circuit].tfstate }}.tfplan \
  -a plan
